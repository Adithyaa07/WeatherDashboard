...
const Middle = (props) => {
  ...
  async function getWeatherData(cityInput, zipCode) {
    if (cityInput) {
      apiUrl = `https://api.openweathermap.org/data/2.5/forecast?q=${cityInput}&limit=5&appid=238a95bf4e0b770d9c0efcbc26a7c178&units=${unit}`;
      if (cityInput === "404" || cityInput === "undefined") {
        alert("Sorry, the city you entered could not be found :(");
      }
    } else if (zipCode) {
      apiUrl = `https://api.openweathermap.org/data/2.5/weather?zip=${zipCode}&appid=238a95bf4e0b770d9c0efcbc26a7c178`;
    } else {
      ...
      apiUrl = `https://api.openweathermap.org/data/2.5/forecast?id=${randomCity.id}&appid=238a95bf4e0b770d9c0efcbc26a7c178&units=${unit}`;
    }
    ...
    try {
      ...
      async function getNearByCities() {
        ...
        try {
          const nearbyCitiesApiUrl = `https://api.openweathermap.org/data/2.5/find?lat=${lat}&lon=${lon}&cnt=5&appid=238a95bf4e0b770d9c0efcbc26a7c178&units=${unit}`;
      ...
      async function getAirPollutionData() {
        try {
          ...
          const response = await fetch(
            `https://api.openweathermap.org/data/2.5/air_pollution?lat=${lat}&lon=${lon}&cnt=5&appid=238a95bf4e0b770d9c0efcbc26a7c178`
          );
